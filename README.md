...
the custom shortcuts from this `.vimrc`. Keep it next to your config
repo as `VIM_COMMANDS.md`.
...
| `Leader ?` | Open this cheat sheet (`~/.vim/VIM_COMMANDS.md` — place this file there for the shortcut to work) |
...
| `Ctrl+f` / `Ctrl+b` | Full page down / up *(remapped to fuzzy search in this config — use `Page Down`/`Page Up` instead)* |
...
| `di(` / `da(` | Delete inside / around `()` (works with `"`, `'`, `{`, `[`, tags via vim-surround) |
...
| `:earlier 5m` | Revert buffer to how it looked 5 minutes ago |
| `:later 5m` | Undo the above |
| `:set ft?` | Show the current filetype |
| `!!ls` (in normal mode via `:.!ls`) | Insert shell command output into buffer |
